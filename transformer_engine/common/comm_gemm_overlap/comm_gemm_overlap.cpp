...
void CommOverlapP2PBase::split_overlap_rs(const TensorWrapper &A, bool transa,
  ...
  for (int i = 0; i < _tp_size; i++) {
    ...
    if (i == 1) {
      ...
      userbuffers_barrier(_ub_reg, _ub_comm, _tp_id, _tp_size, _rank_round_tp, _stream_compute[stream_id]);
      NVTE_CHECK_CUDA(cudaEventRecord(_start_comm, _stream_compute[stream_id]));
